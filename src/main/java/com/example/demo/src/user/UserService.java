...
//            return new PostUserRes(userIdx);
    ...
    public void modifyUserIdName(PatchUserIdReq patchUserIdReq) throws BaseException{
        try {
            int result = userDao.modifyUserIdName(patchUserIdReq); // 해당 과정이 무사히 수행되면 True(1), 그렇지 않으면 False(0)입니다.
            if (result == 0) { // result값이 0이면 과정이 실패한 것이므로 에러 메서지를 보냅니다.
